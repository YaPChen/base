...
public class TagAliasOperatorHelper {
            ...
            switch (msg.what){
                case DELAY_SEND_ACTION:
                    if(msg.obj !=null && msg.obj instanceof  TagAliasBean){
                        DebugUtils.logD(TAG,"on delay time");
                        sequence++;
                        ...
                        if(context!=null) {
                            ...
                            DebugUtils.logE(TAG,"#unexcepted - context was null");
                        }
                    }else{
                        DebugUtils.logD(TAG,"#unexcepted - msg obj was incorrect");
                    }
                    break;
            }
        }
    ...
    public void handleAction(Context context,int sequence, TagAliasBean tagAliasBean){
        ...
        if(tagAliasBean == null){
            DebugUtils.logD(TAG,"tagAliasBean was null");
            return;
        }
        ...
        if(tagAliasBean.isAliasAction){
            switch (tagAliasBean.action){
                ...
                case ACTION_SET:
                    ...
                    break;
                default:
                    DebugUtils.logD(TAG,"unsupport alias action type");
                    return;
            }
        }else {
            switch (tagAliasBean.action) {
                ...
                case ACTION_CLEAN:
                    ...
                    break;
                default:
                    DebugUtils.logD(TAG,"unsupport tag action type");
                    return;
            }
        }
    }
    private boolean RetryActionIfNeeded(int errorCode,TagAliasBean tagAliasBean){
        if(!Utils.isConnected(context)){
            DebugUtils.logD(TAG,"no network");
            return false;
        }
        //返回的错误码为6002 超时,6014 服务器繁忙,都建议延迟重试
        if(errorCode == 6002 || errorCode == 6014){
            DebugUtils.logD(TAG,"need retry");
            if(tagAliasBean!=null){
                ...
                MyApplication.getInstance().showMessage(logs);
                return true;
            }
        }
        return false;
    ...
    public void onTagOperatorResult(Context context, JPushMessage jPushMessage) {
        ...
        DebugUtils.logD(TAG,"action - onTagOperatorResult, sequence:"+sequence+",tags:"+jPushMessage.getTags());
        DebugUtils.logD(TAG,"tags size:"+jPushMessage.getTags().size());
        init(context);
        //根据sequence从之前操作缓存中获取缓存记录
        TagAliasBean tagAliasBean = tagAliasActionCache.get(sequence);
        if(tagAliasBean == null){
            MyApplication.getInstance().showMessage("获取缓存记录失败");
            return;
        }
        if(jPushMessage.getErrorCode() == 0){
            DebugUtils.logD(TAG,"action - modify tag Success,sequence:"+sequence);
            tagAliasActionCache.remove(sequence);
            String logs = getActionStr(tagAliasBean.action)+" tags success";
            DebugUtils.logD(TAG,logs);
            MyApplication.getInstance().showMessage(logs);
        }else{
    ...
    public void onCheckTagOperatorResult(Context context, JPushMessage jPushMessage){
        int sequence = jPushMessage.getSequence();
        DebugUtils.logD(TAG,"action - onCheckTagOperatorResult, sequence:"+sequence+",checktag:"+jPushMessage.getCheckTag());
        init(context);
        //根据sequence从之前操作缓存中获取缓存记录
        TagAliasBean tagAliasBean = tagAliasActionCache.get(sequence);
        if(tagAliasBean == null){
        ...
        if(jPushMessage.getErrorCode() == 0){
            DebugUtils.logD(TAG,"tagBean:"+tagAliasBean);
            tagAliasActionCache.remove(sequence);
    ...
    public void onAliasOperatorResult(Context context, JPushMessage jPushMessage) {
        int sequence = jPushMessage.getSequence();
        DebugUtils.logD(TAG,"action - onAliasOperatorResult, sequence:"+sequence+",alias:"+jPushMessage.getAlias());
        init(context);
        //根据sequence从之前操作缓存中获取缓存记录
        TagAliasBean tagAliasBean = tagAliasActionCache.get(sequence);
        if(tagAliasBean == null){
        ...
        if(jPushMessage.getErrorCode() == 0){
            DebugUtils.logD(TAG,"action - modify alias Success,sequence:"+sequence);
            tagAliasActionCache.remove(sequence);
